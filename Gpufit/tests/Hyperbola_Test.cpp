...
BOOST_AUTO_TEST_CASE( Hyperbola_Fit_1D )
{
	/*
		Performs a single fit using the Hyperbola Fit (HYPERBOLA) model.
		- Uses user info 
	...
	*/
    ...
    std::size_t const n_points{ 4 } ;

	std::array< REAL, 2 > const true_parameters{ { 20, 30, 10, 5, 7 } };

    std::array< REAL, n_points * 3 > data{ { { 1, 2 }, { 1, 2 }, { 1, 2 } } } ;
