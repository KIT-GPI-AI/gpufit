...
void linear_regression_example()
{
    ...
    */
	...
	for (std::size_t i = 0; i != n_fits; i++)
	{
		...
		std::cout << initial_parameters[i * n_model_parameters + 0] << ' ';
		std::cout << initial_parameters[i * n_model_parameters + 1] << ' ';
	}

	// generate data
	std::vector< REAL > data(n_points_per_fit * n_fits);
	for (std::size_t i = 0; i != data.size() * true_parameters.size(); i++) // i is the total element index (n_fits * len(x) * n_parameters)
	{
		std::size_t j = i / n_points_per_fit * true_parameters.size(); // the fit aka ROW ID (fit #)
		std::size_t k = i % n_points_per_fit * true_parameters.size(); // the position within a fit aka COL ID (x position index)
