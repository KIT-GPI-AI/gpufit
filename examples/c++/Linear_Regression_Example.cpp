...
void linear_regression_example()
{
    ...
    */
	...
	for (std::size_t i = 0; i != data.size(); i++) // i is the total element index (n_fits * len(x))
	{
		std::size_t j = i / n_points_per_fit; // the fit - ROW ID (fit #)
		std::size_t k = i % n_points_per_fit; // the position within a fit - COL ID (x position index)
